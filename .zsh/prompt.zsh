...
setprompt () {
    ...
    PROMPT='${PR_BOLD_RED}<${PR_RED}<${PR_BOLD_RED}<${PR_BOLD_WHITE} \
%D{%R.%S %a %b %d %Y}${PR_RED} ☭ ${PR_BOLD_WHITE}%${PR_PWDLEN}<...<%~%<< \

${PR_BOLD_RED}<${PR_RED}<${PR_RED}<\
${PR_BOLD_WHITE} %n@%m${PR_RED} ☭ ${PR_BOLD_RED}\
%(?.. E:%?)${PR_BOLD_BLUE}${PR_SCREEN}${PR_JOBS}${PR_GIT}${PR_BATTERY}\
${PR_YELLOW}${VIMODE}${PR_BOLD_WHITE}\

${PR_GREEN}>${PR_GREEN}>${PR_BOLD_GREEN}>\
%{${reset_color}%} '
